...
public interface ChatMapper {
    ...
    @Select("SELECT * FROM \"ChatMessage\" WHERE \"conversationId\" = #{convId} ORDER BY \"createdAt\" DESC, id DESC LIMIT #{limit} OFFSET #{offset}")
    ...
    @Select("SELECT * FROM \"ChatMessage\" WHERE \"conversationId\" = #{convId} ORDER BY \"createdAt\" DESC, id DESC")
    List<ChatMessage> findMessages(String convId);

    @Insert("INSERT INTO \"ChatMessage\" (id, \"conversationId\", role, content, \"createdAt\") VALUES (#{id}, #{conversationId}, #{role}, #{content}, #{createdAt})")
